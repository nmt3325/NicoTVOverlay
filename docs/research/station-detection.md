...
## 0.1.4: 番組表から全画面へ戻ったときの局の復帰

実機（AQUOS 4K / Android 14）で番組表を開いて BACK で全画面ライブに戻すと、AQUOS はチャンネルコール OSD を
再表示しない。このため 0.1.3 までは局ラベルを読む手がかりがなく、実測で 12 秒以上 `station=null` のままで
コメントが復帰しなかった（読み取れない理由は `AQUOSの全画面ライブを再確認・OSDは非表示`）。

0.1.4 では「直前に確定していた局」を、以下をすべて満たすときに限って暫定復元する。

- 同じテレビアプリのパッケージであること
- 校正済みの全画面ライブ画面で、OSD が一切出ていない読み取りが `RESUME_GRACE_MS`（2.5 秒）以上連続すること
- 直前の確定から `RESTORE_TTL_MS`（90 秒）以内であること
- 確認待ちの候補（pending）がないこと

猛予時間を置くのは安全のため。番組表から選局した場合は AQUOS が直後にチャンネルコール OSD を出すので、
猛予時間内に新しい局が正規に確定し、暫定復元は発生しない。逆に単発のスキャンでは決して復元しないため、
既存の fail-closed 不変条件（Home 復帰・未登録ラベル・番組表・選局キーの直後に古い局を再生しない）を維持できる。

さらに次の場合は記憶自体を破棄する。

- 選局キー（チャンネル±・数字・入力切換・地上/BS/CS など）を検知したとき
- 校正済みラベルを読めたが未登録の局だったとき（別の局が映っている証拠になる）

回帰テストは `AquosOsdTest`（実リーダー経由の番組表シナリオ）と `DetectionPolicyTest`（猛予時間・アプリ跨ぎ・
TTL 失効・OSD 優先）に追加した。既存の
`staleScanGapHomeUnknownStationAndTuningInvalidationNeverReplayOldStation` は弱めずに残し、
単発スキャンでの復元が起きないことを保証している。
